  ...
  return (
    <span
      ...
      dangerouslySetInnerHTML={{
        __html: `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">${svg}</svg>`,
      }}
    />
  );
}

export function Button({ variant = "primary", icon, iconRight, children, ...rest }) {
  return (
...
export function Tag({ plain, children }) {
  return <span className={plain ? "tag-plain" : "tag"}>{children}</span>;
}

export function SectionMarker({ num, label }) {
  return (
...
export function Metric({ pre, value, unit, cap }) {
  return (
...
export function Reveal({ children, as = "div", className = "", ...rest }) {
  ...
  const TagEl = as;
  return (
    <TagEl ref={ref} className={"reveal " + className} {...rest}>
      {children}
    </TagEl>
  );
}
